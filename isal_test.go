...
var (
        textTwain, _ = ioutil.ReadFile("mt.txt")
        textE, _ = ioutil.ReadFile("e.txt")
        text            []byte
)
...
func TestDeflateInflate(t *testing.T) {
	...
	if !(bytes.Equal(n3,buf[:n2])) { t.Errorf("files not same\n") }
	...
	buf3 :=make([] byte, 1024*1024)
        ...
        if !(bytes.Equal(n3,buf3[:n2])) { t.Errorf("files not same\n") }
